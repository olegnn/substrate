...
decl_storage! {
    trait Store for Module<T: Config> as PhragmenElection {
        ...
        ///
        /// Second element is the deposit, third is the block number when candidate was submitted.
        ///
        /// Invariant: Always sorted based on account id.
        pub Candidates get(fn candidates): Vec<(T::AccountId, BalanceOf<T>, T::BlockNumber)>;
...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        /// # </weight>
        #[weight = T::WeightInfo::submit_candidacy(*candidate_count) + T::DbWeight::get().reads_writes(1, 1)]
        fn submit_candidacy(origin, #[compact] candidate_count: u32) {
            ...
            <Candidates<T>>::mutate(|c| c.insert(index, (who, T::CandidacyBond::get(), <frame_system::Module<T>>::block_number())));
        }
        ...
        fn on_runtime_upgrade() -> Weight {
            T::DbWeight::get().reads(1) + if Self::version() == StorageVersion::Initial {
                Version::put(StorageVersion::CandidacyDelay);

                migrations::migrate_to_candidacy_delay::<T>() + T::DbWeight::get().writes(1)
            } else {
                0
            }
        }
...
impl<T: Config> Module<T> {
    ...
    fn do_phragmen() -> Weight {
        ...
        let max_candidate_submission_block =
            <frame_system::Module<T>>::block_number().checked_sub(&T::CandidacyDelay::get());
...
mod tests {
    ...
    parameter_types! {
        ...
        pub CandidacyDelay: <Test as frame_system::Config>::BlockNumber = 4u32.into();
    }
    ...
    fn updating_candidacy_bond_works() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(Elections::candidates(), vec![(5, 3, 1)]);
            ...
            assert_ok!(submit_candidacy(Origin::signed(4)));
            assert_ok!(vote(Origin::signed(4), vec![4], 40));
            assert_eq!(Elections::candidates(), vec![(4, 4, 1), (5, 3, 1)]);
    ...
    fn remove_members_triggers_election() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(members_ids(), vec![5]); // new members
        });
    }
